...
import { Table, Popover, Button, Empty, Modal } from "antd";
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      const updatePromises = selectedRowKeys.map(async (id) => {
        const response = await PurchaseService.updatePayout(id as string, {
          status,
          comment,
        });
        if (response && response.data.success) {
            toast.success(`Payout ${id} updated successfully!`);
        } else {
            toast.error(`Failed to update payout ${id}`);
        }
        return response;
      });
      ...
      setSelectedRowKeys([]);
        ...
        toast.error("Error updating payouts:");
    }
  };
  const columns = [
    {
    ...
    },
    {
      ...
      render: (balance_instructor_received: number) => (
        <div className="text-right">
          {balance_instructor_received.toLocaleString()} VND
        </div>
      ),
    },
    {
